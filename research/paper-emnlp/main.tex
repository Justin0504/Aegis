...
\title{\system: A Cost-Aware Pre-Execution Firewall\\ for Tool-Using LLM Agents}
...
\item \textbf{Cost-aware cascade defense.} A three-layer pipeline (rules $\to$ anomaly detection $\to$ LLM judge) that short-circuits at the cheapest confident layer.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{../../AEGIS_ACM_CAI_/images/framework.pdf}
    \vspace{-0.2in}
    \caption{\system architecture. SDKs intercept tool calls across 14 frameworks. The Gateway runs a three-layer cascade (L1: rules, L2: Isolation Forest, L3: LLM judge). Pending calls route to the Compliance Cockpit for human approval.}
    \vspace{-0.15in}
