...
howToDuration: "PT45M"
howToSteps:
  - "Classify your workload: regulated vertical (fintech / health / gov / EU DPA), data-residency-critical, or general commercial. Regulated workloads bias hard toward self-host; general commercial can start with SaaS and migrate."
  - "For regulated: pick a self-host or open-core project so the enforcement path and audit logs stay in your VPC. Open-core (MIT engine + license-key Pro features) is the durable compromise — you can inspect the engine and vendor still ships updates."
  - "For general commercial: SaaS wins on operational simplicity and detector-update velocity, but audit exportability, data-processing addendum quality, and vendor lock-in cost should still be checked before signing."
  - "In either mode, verify tamper-evident audit — plain database logs fail covered-entity review under HIPAA §164.312(b) and EU AI Act Article 12. Look for RFC 6962-style Merkle logs with independent witness co-signature."
  - "Model total cost across 3 years: SaaS per-call pricing typically becomes uneconomic above ~100k calls/mo; self-host has infra + team cost but predictable at scale. Model both curves before committing."
  - "For the open-core hybrid pattern (AEGIS ships this): engine is MIT + self-hosted; a paid license key unlocks Pro-tier detectors, SSO, longer retention, and support. The engine keeps running with the base tier even if you cancel — no lock-in."
keyTakeaways:
