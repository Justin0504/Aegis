...
export function ProxyWizard() {
  ...
  const [binResult, setBinResult]       = useState<SysCmdResult | null>(null)
  const [binRunning, setBinRunning]     = useState(false)
  ...
  useEffect(() => {
    ...
    if (tauri) {
      ...
      tauriInvoke<SysCmdResult>('proxy_status')
        .then((r) => { setBinRunning(r.ok); setBinResult(r) }).catch(() => {})
    }
  }, [])
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      {/* ── Step 2: start the proxy binary ────────────────────────── */}
      <Step
        n={2}
        title="Start the AEGIS proxy"
        subtitle="Spawns the transparent MITM binary on 127.0.0.1:18081. Traffic to allowlisted hosts is decrypted + traced; everything else tunnels through untouched."
      >
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={async () => {
              setRunning('start-proxy')
              try {
                const r = await tauriInvoke<SysCmdResult>('proxy_start')
                setBinResult(r); setBinRunning(r.ok)
              } catch (e: any) {
                setBinResult({ ok: false, command: 'proxy_start', stdout: '', stderr: e.message ?? String(e) })
              } finally { setRunning(null) }
            }}
            disabled={!inTauri || running !== null || binRunning}
            ...
            style={{
              ...
              opacity: !inTauri || running !== null || binRunning ? 0.5 : 1,
            }}>
            {running === 'start-proxy'
              ? (<><Loader2 className="h-3.5 w-3.5 animate-spin" /> Starting…</>)
              : (<><Zap className="h-3.5 w-3.5" /> {binRunning ? 'Proxy running' : 'Start proxy'}</>)}
          </button>
          <button
            onClick={async () => {
              setRunning('stop-proxy')
              try {
                const r = await tauriInvoke<SysCmdResult>('proxy_stop')
                setBinResult(r); setBinRunning(false)
              } catch (e: any) {
                setBinResult({ ok: false, command: 'proxy_stop', stdout: '', stderr: e.message ?? String(e) })
              } finally { setRunning(null) }
            }}
            disabled={!inTauri || running !== null || !binRunning}
            className="text-xs px-2.5 py-1.5 rounded border flex items-center gap-1"
            style={{ borderColor: BORDER, color: MUTED, opacity: binRunning ? 1 : 0.5 }}>
            <ShieldOff className="h-3 w-3" />
            Stop
          </button>
        </div>
        {binResult && <CmdOutput result={binResult} />}
      </Step>

      {/* ── Step 3: install CA cert ───────────────────────────────── */}
      <Step
        n={3}
